...
/*
* 在 window 对象中比较重要的几个方法
*   1.document: 文档对象, 用于操作HTML元素
*   2.navigator: 导航对象, 用于获取浏览器信息（用于反爬，发现用户是否正常在浏览器中访问）
*   3.location: 位置对象, 用于获取当前页面的URL信息
*   3.frames: 框架对象, 用于操作框架和查询框架
*   4.history: 历史对象, 用于操作浏览器的历史记录
*   5.screen: 屏幕对象, 用于获取屏幕信息（获取当前屏幕的分辨率、如果开启了控制台，就可以被发现，控制台打开后屏幕分辨率变化了）
* 以上几个对象需要在浏览器控制台执行
* */

/**
 * 浏览器中看到的 window 在 node 解释器中式不存在的。
 * 补环境中，破解文件是在node执行的，在浏览器扣的代码无法在node环境中执行。
 * 改 node 中的 global 可以完成一部分的补环境。
 */